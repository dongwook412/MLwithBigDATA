...
class CNN():
  ...
  def __init__(self):
    ...
    self.y_ = tf.placeholder(tf.float32, [None, 100],name='y')
    with tf.name_scope('reshape'):
      self.x_image = tf.reshape(self.x, [-1, 32, 32, 3],name='x_image')
    with tf.name_scope('conv1'):
      self.W_conv1 = weight_variable([5, 5, 3, 20],'W_conv1')
    ...
    with tf.name_scope('fc1'):
      self.W_fc1 = weight_variable([5 * 5 * 50, 500],'W_fc1')
    ...
    with tf.name_scope('fc2'):
      self.W_fc2 = weight_variable([500, 100],'W_fc2')
